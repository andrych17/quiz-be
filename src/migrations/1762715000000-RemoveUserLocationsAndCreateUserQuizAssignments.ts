...
export class RemoveUserLocationsAndCreateUserQuizAssignments1762715000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE INDEX "IDX_user_quiz_assignment_userId" ON "user_quiz_assignments" ("userId")`);
        await queryRunner.query(`CREATE INDEX "IDX_user_quiz_assignment_quizId" ON "user_quiz_assignments" ("quizId")`);
        await queryRunner.query(`CREATE INDEX "IDX_user_quiz_assignment_active" ON "user_quiz_assignments" ("isActive")`);
        ...
        let userRoleEnum = enumTypesWithValues.find((type: any) => 
        ...
        if (!userRoleEnum && enumTypesWithValues.length > 0) {
            ...
            userRoleEnum = enumTypesWithValues.find((type: any) => type.typname.includes('role'));
        }
        ...
        if (userRoleEnum) {
            ...
            console.log('Current enum values:', roleEnumValues.map((r: any) => r.enumlabel));

            const hasSupeadmin = roleEnumValues.some((row: any) => row.enumlabel === 'superadmin');
            
            if (!hasSupeadmin) {
                // Add 'superadmin' to the enum
                await queryRunner.query(`ALTER TYPE "${enumTypeName}" ADD VALUE 'superadmin'`);
                console.log(`Added superadmin role to enum ${enumTypeName}`);
            } else {
                console.log('Superadmin role already exists');
            }
        } else {
            console.log('No suitable role enum found, skipping superadmin role addition');
        }

        // Step 6: Drop foreign key constraints from user_locations
        await queryRunner.query(`ALTER TABLE "user_locations" DROP CONSTRAINT IF EXISTS "FK_user_location_config"`);
        await queryRunner.query(`ALTER TABLE "user_locations" DROP CONSTRAINT IF EXISTS "FK_user_location_user"`);
        ...
        console.log('- Migrated admin-location assignments to admin-quiz assignments');
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "user_quiz_assignments" DROP CONSTRAINT IF EXISTS "FK_user_quiz_assignment_quiz"`);
        await queryRunner.query(`ALTER TABLE "user_quiz_assignments" DROP CONSTRAINT IF EXISTS "FK_user_quiz_assignment_user"`);
