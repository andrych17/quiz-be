...
export class AddServiceIdToUsersAndQuizzes1762730000000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (usersTableExists) {
      const serviceIdColumnExists = await queryRunner.hasColumn(
        'users',
        'serviceId',
      );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Remove columns
    await queryRunner.query(
      `ALTER TABLE "users" DROP COLUMN IF EXISTS "serviceId";`,
    );
    await queryRunner.query(
      `ALTER TABLE "quizzes" DROP COLUMN IF EXISTS "serviceId";`,
    );
