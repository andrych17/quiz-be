...
export class InitialMigration1731686400000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "service_type" AS ENUM ('service-management', 'network-management', 'database-admin', 'system-admin', 'web-development', 'mobile-development', 'data-science', 'cybersecurity', 'cloud-computing', 'devops')`,
    );

    // Create question type enum
    await queryRunner.query(
      `CREATE TYPE "question_type" AS ENUM ('multiple-choice', 'multiple-select', 'text')`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_config_group" ON "config_items" ("group")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_quiz_slug" ON "quizzes" ("slug")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_quiz_serviceType" ON "quizzes" ("serviceType")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_quiz_locationId" ON "quizzes" ("locationId")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_question_quiz_order" ON "questions" ("quizId", "order")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_attempt_quiz" ON "attempts" ("quizId")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_attempt_email" ON "attempts" ("email")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_attempt_nij" ON "attempts" ("nij")`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_answer_attempt_question" ON "attempt_answers" ("attemptId", "questionId")`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user_locations" DROP CONSTRAINT "FK_user_location_config"`,
    );
    await queryRunner.query(
      `ALTER TABLE "user_locations" DROP CONSTRAINT "FK_user_location_user"`,
    );
    await queryRunner.query(
      `ALTER TABLE "attempt_answers" DROP CONSTRAINT "FK_answer_question"`,
    );
    await queryRunner.query(
      `ALTER TABLE "attempt_answers" DROP CONSTRAINT "FK_answer_attempt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "attempts" DROP CONSTRAINT "FK_attempt_quiz"`,
    );
    await queryRunner.query(
      `ALTER TABLE "questions" DROP CONSTRAINT "FK_question_quiz"`,
    );
    await queryRunner.query(
      `ALTER TABLE "quizzes" DROP CONSTRAINT "FK_quiz_location"`,
    );
